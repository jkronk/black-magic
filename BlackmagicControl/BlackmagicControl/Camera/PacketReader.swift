...
class PacketReader {
    weak var m_packetDecodedDelegate: PacketDecodedDelegate?

    func readCCUPacket(_ data: Data?) {
        if data != nil {
            do {
                if m_packetDecodedDelegate != nil {
                    try CCUDecodingFunctions.DecodeCCUPacket(data: data!, respondTo: m_packetDecodedDelegate!)
                }
            } catch {
    ...
    func readTimecodePacket(_ data: Data?) {
        if data != nil {
            ...
            m_packetDecodedDelegate?.onTimecodeReceived(timecode)
        } else {
            Logger.LogNoPacketDataWarning(packetType: "Timecode")
        }
    }

    func readCameraStatus(_ data: Data?) -> UInt8 {
        return CameraStatus.GetCameraStatusFlags(data)
    }

    func readCameraModelPacket(_ data: Data?) {
        if data != nil {
